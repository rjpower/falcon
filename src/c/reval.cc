...
struct GilHelper {
  ...
  GilHelper() : state_(PyGILState_Ensure()) {}
  ~GilHelper() {
...
#define _DEFINE_OP(opname, impl){\
      ...
      switch (status) {\
        case EVAL_CONTINUE:\
        fprintf(stderr, "Jumping to: %d, %s\n", state.offset, opcode_to_name(next.code()));\
...
PyObject* RegisterEvalFrame(RegisterFrame *reg_frame) {
  ...
  fprintf(stderr, "Jumping to: %ld, %s\n", state.offset, opcode_to_name(next.code()));
  ...
  BAD_OP(POP_TOP);
  BAD_OP(STOP_CODE);
  op_BADCODE: { fprintf(stderr, "Bad opcode\n."); abort();}
  error:
    fprintf(stderr, "error during evaluation...\n");
    return NULL;
  done:
    fprintf(stderr, "Leaving eval frame...\n");
    return state.result;
}
